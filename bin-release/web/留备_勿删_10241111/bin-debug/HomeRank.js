var HomeRank = (function (_super) {
    __extends(HomeRank, _super);
    function HomeRank(w, h, rate) {
        _super.call(this);
    ...
    var d = __define,c=HomeRank;p=c.prototype;
    ...
    p.onBackToMain = function (evt) {
        ClickEvent('SHCrarus', 'SHCrarus/rarus_List_return_Click', 'Click');
        this.dispatchEvent(new egret.Event('hideRank', true));
    };
    p.addGreyT = function () {
    ...
    p.introAnim = function () {
        ...
        egret.Tween.get(this.grayT).wait(500).to({ scaleX: 1, scaleY: 1, x: -10, y: 6 }, 200, egret.Ease.backOut);
        egret.Tween.get(this.btnBack).wait(700).to({ y: -136 }, 250, egret.Ease.backOut);
        egret.Tween.get(this.redT).wait(800).to({ scaleX: 1, scaleY: 1, x: 510, y: -186, alpha: 1 }, 500, egret.Ease.elasticOut);
    };
    p.readList = function () {
    ...
    p.onLoadComplete = function (evt) {
        ...
        for (var i = 0; i < lDataLength; i++) {
            var perRank = new PerRank(i + 1, lData.split('~|')[i].split(',')[0], Number(lData.split('~|')[i].split(',')[1]), false);
...
egret.registerClass(HomeRank,"HomeRank");
