class HomeRank extends egret.Sprite {
    private _iW: number;
    private _iH: number;
    ...
    private introAnim(): void {
        ...
        egret.Tween.get( this.grayT )
            .wait( 500 )
            .to( { scaleX: 1, scaleY: 1, x: -10, y: 6 }, 200, egret.Ease.backOut );
        ...
        egret.Tween.get( this.redT )
            .wait( 800 )
            .to( { scaleX: 1, scaleY: 1, x: 510, y: -186, alpha: 1 }, 500, egret.Ease.elasticOut);
    }
    ...
    private onLoadComplete( evt: egret.Event ): void {
        ...
        for( var i: number = 0; i < lDataLength; i++ ) {
            var perRank: PerRank = new PerRank( i + 1, lData.split( '~|' )[i].split(',')[0], Number( lData.split( '~|' )[i].split(',')[1] ), false );
